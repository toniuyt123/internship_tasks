...
class RoadSpeed:
    ...
    def speed(self, n, roads):
        ...
        for path in paths:
            ...
            for p in path:
                ...
                for i in range(len(p) - 1):
                    ...
                    if len(speeds) > 1:
                        temp.append(speeds[dup_index[str(p)] % len(speeds)])
                        dup_index[str(p)] += 1
                    else:
        ...
        print(speeds)
        for speed in speeds:
            max_speed = max([min(s) for s in speed])
            min_speed = min([max(list(filter(lambda x: x <= max_speed, s))) for s in speed])
